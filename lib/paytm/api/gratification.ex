defmodule Paytm.API.Gratification do
  ...
  def credit(%Money{amount: amount, currency: :INR}, order_id, phone, email, options \\ []) do
    ...
    |> case do
      ...
      {:ok,
       %{"status" => "PENDING", "statusCode" => _, "statusMessage" => _, "response" => response}} ->
        {:ok, response, :pending}

      {:ok, %{"statusCode" => code, "statusMessage" => message}} ->
  ...
  def check_status(reference_type, reference) do
    ...
    |> handle_response
    |> case do
      {:ok, %{"status" => "SUCCESS", "statusMessage" => "SUCCESS", "response" => response}} ->
