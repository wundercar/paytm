defmodule Paytm.API.Wallet do
  ...
  def fetch_balance(%Token{access_token: access_token}) do
    ...
    |> case do
      ...
      {:ok, %{"status" => "FAILURE", "statusCode" => code, "statusMessage" => message}} ->
  ...
  def add_money(
        %Money{amount: amount, currency: :INR},
        order_id,
        customer_id,
  ...
  @spec charge(
          ...
          | {:error, message :: String.t(), code :: String.t() | atom | nil,
             transaction :: Transaction.t() | nil}
  def charge(
        %Money{amount: amount, currency: :INR},
        order_id,
        customer_id,
        %Token{access_token: token, phone: phone},
        options \\ []
      ) do
    params = %{
      MID: config(:merchant_id),
    ...
    |> case do
      ...
      _ ->
        {:error, "An unknown error occurred", nil, nil}
    end
  end
