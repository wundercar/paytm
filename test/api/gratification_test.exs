defmodule Paytm.API.GratificationTest do
  ...
  setup do
    ExVCR.Config.filter_sensitive_data(
      ~s("merchantGuid":".*?"),
      ~s("merchantGuid":"MERCHANT_GUID")
    )

    ExVCR.Config.filter_sensitive_data(
      ~s("salesWalletGuid":".*?"),
      ~s("salesWalletGuid":"SALES_WALLET_GUID")
    )

    ExVCR.Config.filter_sensitive_data(
      ~s("merchantOrderId":".*?"),
      ~s("merchantOrderId":"foobar")
    )
  ...
  describe "credit/5" do
    ...
    test "successfully credits an existing user's wallet" do
      use_cassette "api-gratification-credit-successful-credit",
        match_requests_on: [:request_body] do
        assert {:ok, _response, :success} =
    ...
    test "returns :error on invalid data" do
      use_cassette "api-gratification-credit-unsuccessful-credit",
        match_requests_on: [:request_body] do
        assert {:error, _, "GE_1011"} =
                 Gratification.credit(
                   ...
                   Helpers.random_id(),
                   "not_a_phone_number",
                   "invalid_email_foo_bar"
                 )
      end
    end

    test "doesn't normally allow crediting of a new user" do
      use_cassette "api-gratification-credit-unsuccessful-new-user-credit",
        match_requests_on: [:request_body] do
        assert {:error, _, "STUC_1002"} =
                 Gratification.credit(
                   Money.new(10_00, :INR),
                   Helpers.random_id(),
                   Helpers.random_phone(),
                   "#{Helpers.random_id()}@foobar.com"
                 )
      end
    end

    test "allows crediting of a new user when apply_to_new_users is true" do
      use_cassette "api-gratification-credit-successful-new-user-credit",
        match_requests_on: [:request_body] do
  ...
  describe "check_status/2" do
    test "returns details about an actual transaction" do
      use_cassette "api-gratification-successful-check-status", match_requests_on: [:request_body] do
        ...
        assert {:ok, [%Transaction{id: id, status: :pending}]} =
                 Gratification.check_status(:order_id, order_id)
    ...
    test "returns an error for invalid transactions" do
      use_cassette "api-gratification-unsuccessful-check-status",
        match_requests_on: [:request_body] do
        assert {:error, "Invalid transaction id.", "GE_1009"} =
                 Gratification.check_status(:order_id, Helpers.random_id())
      end
    end
  end
end
